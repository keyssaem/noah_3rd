...
- ✅ **Phase 2 — 🎈 기억 풍선 → 🪜 약속의 계단으로 개편 (2026-07-12)** (`Mini.stairsGame()`): 카메라(손 인식) 버전 기억 풍선은 **제거** — 핀치가 탭 대비 더해주는 게 '신기함'뿐인데 모델 7.5MB·조명 편차·구형기기 리스크를 지불(❗가위바위보는 카메라가 서사 그 자체라 **유지**). 대체: **약속의 계단** — CSS 아이소메트릭 계단 9칸(개별 타일 rotateX57°+rotateZ45°, perspective/preserve-3d 없음 → 구형기기 안전, DOM ~20개). 문제 카드는 **화면 상단**(계단 전경을 가리지 않게, 선생님 지정), 뜻→이름 고르면 계단 점등+캐릭터 등반. 1~3라운드 2지선다→이후 3지선다. 오답=계단 흔들림+그 선택지 제거(자비 규칙, **하강 없음**)+이번 칸 ⭐상실. 첫 시도 정답=⭐(`State.stairStars` 0~9, 헌장 뱃지 연계 후보 — 구 `balloonStars` 대체). 꼭대기에서 노아와 하이파이브 🙌 + `friendDef` 치환 대사("'보물' 같은 친구 덕분에..."). 서사: 오프닝 연구소 트럭 카드 수집과 수미상관. 위치: 5-5 노아 고백 직후(동일). 씬은 설계좌표 780×480을 scale로 화면 대응(즉시 fit + resize 추종)
...
- **남은 것**: 선생님/친구 NPC GLB(친구는 머리색·옷색 틴트로 여러명), 운동장/교실/복도 기본 도형 확장. ⚠ 안정성: examples/js CDN 실패 시 박스 폴백 동작하나, 심사 대비 three+loader 로컬 벤더링 권장. ⚠ GLB는 http 서버/https 필수 — file:// 더블클릭 시 로컬 fetch 차단되어 박스 폴백(뷰어는 `Assets.failHint()`로 안내)
